        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
          {featuredServices.map((service, index) => (
            <AnimatedSection key={service.title} delay={index * 100}>
              <Card className="h-full hover:shadow-lg hover:border-[#2563EB]/30 transition-all duration-300 group">
                <CardContent className="p-6">
                  <h3 className="text-xl font-semibold text-[#0F172A] mb-3 group-hover:text-[#2563EB] transition-colors duration-200">
                    {service.title}
                  </h3>
                  ...
                  <ul className="space-y-2">
                    {service.features.map((feature) => (
                      <li key={feature} className="flex items-center text-sm text-[#64748B]">
                        <span className="w-1.5 h-1.5 rounded-full bg-[#2563EB] mr-3 flex-shrink-0" />
                        {feature}
                      </li>
                    ))}
                  </ul>
